...
const sideDrawer = ({ close, show }) => {
  ...
  const listItemsData = [
    {id:1,text:'Provide Delivery Services', route:'/deviveryservices', icon:'deliver', alert:false},
    {id:2,text:'Sell goods for Delivery', route:'/sell', icon:'sell', alert:false},
    {id:3,text:'FAQs', route:'/faq', icon:'faq', alert:false},
    {id:4,text:'Log Out', route:'/logout', icon:'exit', alert:true}
  ];
  const listItems = listItemsData.map(li=><ListItem key={li.id} text={li.text} route={li.route} icon={li.icon} alert={li.alert}/>)
  return (
